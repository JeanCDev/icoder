...
export class ConfigHelper extends EventEmitter {
  ...
  private defaultConfig: Config = {
    ...
    extractionModel: "gemini-flash-lite-latest", // Default to Flash for faster responses
    solutionModel: "gemini-flash-lite-latest",
    debuggingModel: "gemini-flash-lite-latest",
    language: "python",
    opacity: 1.0
  };
  ...
  private sanitizeModelSelection(model: string, provider: "openai" | "gemini" | "anthropic"): string {
    if (provider === "openai") {
      ...
      // Only allow gemini-2.5-pro and gemini-flash-lite-latest for Gemini
      const allowedModels = ['gemini-2.5-pro', 'gemini-flash-lite-latest'];
      if (!allowedModels.includes(model)) {
        console.warn(`Invalid Gemini model specified: ${model}. Using default model: gemini-flash-lite-latest`);
        return 'gemini-flash-lite-latest'; // Changed default to flash
      }
      return model;
    ...
    try {
      ...
      if (updates.apiProvider && updates.apiProvider !== currentConfig.apiProvider) {
        if (updates.apiProvider === "openai") {
          ...
          updates.extractionModel = "gemini-flash-lite-latest";
          updates.solutionModel = "gemini-flash-lite-latest";
          updates.debuggingModel = "gemini-flash-lite-latest";
        }
      }
